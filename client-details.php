...
{
	$sqlDlt= "delete from tblClientBankMaster where RecID = '".$_POST["client_bank_id"]."' ";
	$resultDlt = mysql_query ($sqlDlt) or die ("Error in  query : ".$sqlDlt."<br>".mysql_errno()." : ".mysql_error());
	$message="<b>".$_POST["txtAccHolderName"]."</b> Successfully Deleted!!";
}

	$flagPost = 0;
	$clientid = $_GET["cid"];
	$sql= "select * from tblClientMaster where ClientID=".$clientid;
...
<table width="100%" cellpadding="5" cellspacing="0" align="center">
    ...
    <tr>
    	<td class="slipHead">Client Details</td>
    </tr>
    <tr>
    	<td align="center" valign="top">
        	<table width="100%" cellpadding="10" cellspacing="0" align="center" border="0" class="myTableR">
            	<tr bgcolor="#DDEBF9">
                	<td class="txtLeft" style="font-size:16px; text-align:center; font-weight:bold;" width="15%" colspan="4"><?=$row["ClientName"]?></td>
                </tr>
            	<tr>
                	<td class="txtLeft" width="15%">Email</td>
                    <td class="txtLeft" width="35%"><?=$row["Email"]?></td>
                    <td class="txtLeft" width="15%">Code</td>
                    <td class="txtLeft" width="35%"><?=$row["ClientCode"]?></td>
                </tr>
                <tr>
                	<td class="txtLeft">PAN No.</td>
                    <td class="txtLeft"><?=$row["PANNo"]?></td>
                    <td class="txtLeft">Contact No.</td>
                    <td class="txtLeft"><?=$row["Phone"]?><? if($row["MobileNo"]!=''){echo ",&nbsp;".$row["MobileNo"];} ?></td>
                </tr>
                <tr>
                    <td class="txtLeft">Address</td>
                    <td class="txtLeft"><?=$row["Address"]?><? if($row["City"]!=''){echo ",&nbsp;".$row["City"];} ?></td>
                    <td class="txtLeft">Father's Name</td>
                    <td class="txtLeft"><?=$row["FatherName"]?></td>
                </tr>
            </table>
        </td>
    </tr>
    <tr>
    	<td align="center" valign="top">
        	<table width="100%" cellpadding="10" cellspacing="0" align="center" border="0">
                <?
					$sqlB = "select * from tblClientBankMaster where ClientID=".$clientid." and ClientType like 'Head' ";
                   ...
                   ?>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" align="left" border="0" class="myTableR">
                           <tr>
                                ...
                                <a href="javascript:;" onClick="document.getElementById('divBank<?=$rowB["RecID"]?>').style.visibility = 'visible'; get_frm('ajxEditClientBank.php','<?=$rowB["RecID"]?>','divBank<?=$rowB["RecID"]?>')"><img src="images/iconEdit.gif"/></a> &nbsp; 
                               	<a href="javascript:;" onClick="document.getElementById('divBank<?=$rowB["RecID"]?>').style.visibility = 'visible'; get_frm('ajxDeleteClientBank.php','<?=$rowB["RecID"]?>','divBank<?=$rowB["RecID"]?>')"><img src="images/iconDelete.gif"/></a>
                                </div>
                                </td>
                            </tr>
                            <tr>
    ...
    <tr>
    	<td align="center" valign="top">
        	<table width="100%" cellpadding="10" cellspacing="0" align="center" border="0">
                <?
					$sqlS = "select * from tblSubClientMaster where ClientID=".$clientid." order by ClientName  ";
					$resultS = mysql_query ($sqlS) or die ("Error in  query : ".$sqlS."<br>".mysql_errno()." : ".mysql_error());
					if(mysql_num_rows($resultS)>0)
					{
				?>
                <tr bgcolor="#e2e2e2">
                    <td class="txtLeft" colspan="4"><span>Sub Client</span></td>
                </tr>
                <tr>
					<?
                    $c = 1;
                    ...
                    <td align="left">
                        <table width="100%" cellpadding="0" cellspacing="0" align="left" border="0" class="myTableR">
                           <tr>
                                ...
                                <td class="txtLeft"><b><?=$rowS["ClientName"]?></b></td>
                            </tr>
                            <tr>
                                <td class="txtLeft">Email ID :</td>
                                <td class="txtLeft"><?=$rowS["EmailID"]?></td>
                            </tr>
                            <tr>
                                <td class="txtLeft">Contact No. :</td>
                                <td class="txtLeft"><?=$rowS["PhoneNo"]?> &nbsp; <?=$rowS["MobileNo"]?></td>
                            </tr>
                            <tr>
                                <td class="txtLeft">PAN No. :</td>
                                <td class="txtLeft"><?=$rowS["PANNo"]?></td>
                            </tr>
                            <tr>
                                <td class="txtLeft">Address :</td>
                                <td class="txtLeft"><?=$rowS["Address"]?> &nbsp; <?=$rowS["City"]?></td>
                            </tr>
                        </table>
                    </td>
               		<?
